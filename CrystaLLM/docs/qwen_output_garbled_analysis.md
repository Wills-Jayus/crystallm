\# Qwen 输出乱码问题复盘（历史记录：早期 LLaMA 阶段）
...
\- 在 `experiments/na2cl2\_json\_loop\_v7\_Large/round\_01/qwen\_output.json` 与 `round\_02/qwen\_output.json` 中，`analysis` 字段均为回退文案“LLM 输出不符合要求，使用回退。”，`next\_prompt\_lines` 被原样复制为上一轮 prompt（`data\_Na2Cl2` + `\_symmetry\_space\_group\_name\_H-M   P4/mmm`），说明模型原始回复无法解析成 JSON。（注：该实验当时文件名为 `llama_output.json`，现统一记为 `qwen_output.json`）
...
\- Round 1（`experiments/na2cl2\_json\_loop\_v7\_Large/round\_01/qwen\_output.json`）
...
\- Round 2（`experiments/na2cl2\_json\_loop\_v7\_Large/round\_02/qwen\_output.json`）
...
\- 本地服务端：`model/qwen\_local\_api.py`（旧名：`model/llama\_local\_api.py`）
