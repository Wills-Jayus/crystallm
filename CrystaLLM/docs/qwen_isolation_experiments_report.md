\# Qwen 本地部署隔离实验报告（历史记录：早期 LLaMA 阶段）
...
在完整 CrystaLLM + ALIGNN + Qwen 闭环中，我们观察到优化器始终返回回退文案（“LLM 输出不符合要求，使用回退。” / "LLM output invalid, using fallback."），`next\_prompt\_lines` 一直保持不变，说明模型原始输出无法被解析为合法输出。本报告在 `sandbox/` 目录下设计了两组\*\*隔离实验\*\*，目的是：
