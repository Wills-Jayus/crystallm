...
\- 服务端 `llama\_local\_api.py` 也改为默认返回原始生成（`LLAMA\_STRICT\_JSON=false`），不再强制 JSON 解析，增加了尝试在 `END` 处停的逻辑。



\## 已做的代码改动（关键点）

\- `bin/llama\_client.py`：

&nbsp; - System prompt 改为英文模板，禁止回显用户 JSON，只允许三段输出，并给了示例。

&nbsp; - 本地解析 `\_parse\_template` 读取三段，失败则回退，附 `\_raw`。

\- `llama\_local\_api.py`：

&nbsp; - 默认关闭严格 JSON（通过环境变量 `LLAMA\_STRICT\_JSON` 控制）；关闭时直接返回模型原始生成。

&nbsp; - 追加 `/root/autodl-tmp/model/CrystaLLM/llama\_raw\_generation.log` 记录 `generated\_text` 原文。
...
PYTHONPATH=. LLAMA\_STRICT\_JSON=false uvicorn llama\_local\_api:app --host 0.0.0.0 --port 8000
...
import llama\_local\_api, sys

print("path:", llama\_local\_api.\_\_file\_\_)
...
&nbsp;   --input-json experiments/na2cl2\_json\_loop\_v8\_Large/round\_02/llama\_output.json \\
...
\- `llama\_raw\_generation.log` 也只记录了同样的乱码段落。
...
1\. \*\*实现真正的 stopping criteria\*\*：在 `llama\_local\_api.py` 添加自定义 `StoppingCriteria` 匹配完整 `\\nEND` 序列，而不是只追加单个 eos token。
...
4\. \*\*确认加载路径\*\*：重启前先打印 `llama\_local\_api.\_\_file\_\_`，确保确实使用仓库文件。
