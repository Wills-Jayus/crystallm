...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize MP-20 official-test match metrics.")
    ...
    rows_raw = {k: parse_metric(Path(path)) for k, path in rows_paths.items() if path}
    ...
    if rows_delta:
        corresponding = [rows_delta[key] for key, delta in deltas.items() if delta >= 1.0 and key in rows_delta]
    ...
    lines = [
        f"# MP-20 {args.system_id} Official Test",
        "",
