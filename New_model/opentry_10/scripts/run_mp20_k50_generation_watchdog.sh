...
PROMPTS="$OP/generations/mp20_test_gt_sg_k50/prompts/data_atomtype_gt_sg"
RAW="$OP/generations/mp20_test_gt_sg_k50/cifs_raw/data_atomtype_gt_sg"
POST="$OP/generations/mp20_test_gt_sg_k50/cifs_post/data_atomtype_gt_sg"
LOGDIR="$OP/logs/mp20_k50_generation"
...
MODEL="$ROOT/model/scp_task/CrystaLLM/crystallm_benchmarkmodel/cif_model_mp_20_b"
NUM_PROMPTS=9046
EXPECTED_PER_BLOCK=90460
...
run_postprocess() {
  ...
  "$PY" "$POSTPROCESS" "$RAW" "$POST" --workers 96 --resume > "$LOGDIR/postprocess_${tag}.log" 2>&1
}
...
run_generation_block() {
  ...
  CUDA_VISIBLE_DEVICES=0,1 PYTHONDONTWRITEBYTECODE=1 "$PY" "$GEN" \
    ...
    --prompts-dir "$PROMPTS" \
    --out-dir "$RAW" \
    --start-index 0 --num-prompts "$NUM_PROMPTS" \
...
run_generation_block 20 7337 0.7 21 30
