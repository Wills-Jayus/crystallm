...
SYSTEM_ID = "mpts52_k50_rf_seed1_margin_route"
THRESHOLD = 0.12608004456999994
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            src = json.loads(line)
            source_rank = src.get("source_rank", src.get("rank"))
            rank = int(source_rank) + 1 if int(source_rank) == 0 or src.get("source_rank") is not None else int(source_rank)
...
def supplemental_records(post_dir: Path, sample_by_material: dict[str, str], *, max_rank: int) -> list[dict[str, Any]]:
    ...
    *,
    ...
    for col in feature_cols:
        if col not in df.columns:
            df[col] = None
    df["score"] = pipe.predict_proba(df[feature_cols])[:, 1]
    ...
    for sample_id, group in df.groupby("sample_id", sort=False):
        ...
        if route_rule == "best_minus_rank1_ge":
            route_value = float(best_score - rank1_score)
        elif route_rule == "best_score_ge":
            route_value = float(best_score)
        else:
            raise ValueError(f"unknown route_rule: {route_rule}")
            ...
            selected_rows.append(
                {
                    ...
                    "route_decision": "rf_sort" if should_route else "baseline_k20",
                }
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build frozen MPTS-52 K50 RF margin-route official-test outputs.")
    ...
    parser.add_argument("--anchor-k20", default=str(ROOT / "candidates/unified_mpts_52_test_crystallm_a_gt_sg_anchor.jsonl"))
    parser.add_argument("--supplemental-post-dir", default=str(ROOT / "generations/mpts52_test_gt_sg_k50/cifs_post/data_atomtype_gt_sg"))
    parser.add_argument("--test-true-tar", default=str(ROOT / "generations/mpts52_official_test/tars/true.tar.gz"))
    parser.add_argument("--system-id", default=SYSTEM_ID)
    parser.add_argument("--max-rank", type=int, default=50)
    parser.add_argument("--route-rule", default="best_minus_rank1_ge", choices=["best_minus_rank1_ge", "best_score_ge"])
    parser.add_argument("--route-threshold", type=float, default=THRESHOLD)
    parser.add_argument("--route-threshold-source", default="validation OOF q0.7 for rf seed1")
    ...
    model_path = frozen_dir / "ranker_rf_seed1.joblib"
    ...
    supplemental = supplemental_records(Path(args.supplemental_post_dir), sample_by_material, max_rank=int(args.max_rank))
    all_rows = anchors + supplemental
    ...
    selected_rows, selection_manifest, scored_df = select_outputs(
        ...
        route_rule=str(args.route_rule),
