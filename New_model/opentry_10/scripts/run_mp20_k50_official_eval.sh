...
SYSTEM_ID="${1:-mp20_k50_hgb_mean_seed012_margin_route}"

GEN_TAR="$OP/generations/${SYSTEM_ID}_test_k20/tars/generated_data_atomtype_gt_sg.tar.gz"
TRUE_TAR="$OP/generations/mp20_official_test/tars/true.tar.gz"
...
run_metric 5 "$GEN_TAR" "$TRUE_TAR" "$OUT/${SYSTEM_ID}_k5.raw.txt" 40 off "$OUT/${SYSTEM_ID}_k5_unmatched"
...
PYTHONDONTWRITEBYTECODE=1 "$PY" "$OP/scripts/summarize_mp20_official_eval.py" \
  --system-id "$SYSTEM_ID" \
