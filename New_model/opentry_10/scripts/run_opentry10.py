...
LOG_DIR = ROOT / "logs"
RUN_ROOT = ROOT / "generations/crystallm_gt_sg_val_anchor_symprec0p1"
SHARD_ROOT = ROOT / "generations/crystallm_gt_sg_val_anchor_symprec0p1_shards"
BENCH_CIF_ROOT = ROOT / "cache/official_benchmark_cifs_symprec0p1"
...
def action_prepare_validation_gt_cifs() -> None:
    script = ROOT / "scripts/prepare_validation_gt_cifs_symprec.py"
    cmd = [
        str(PY),
        str(script),
        "--split",
    ...
    run_logged("prepare_validation_gt_cifs_symprec0p1", cmd, max_attempts=1)


def val_run_name(dataset: str) -> str:
    return (
        "mp20_val_data_atomtype_gt_sg_symprec0p1_k100"
        if dataset == "mp_20"
        else "mpts52_val_data_atomtype_gt_sg_symprec0p1_k100"
    )
...
def shard_plan_path(dataset: str) -> Path:
    return STATE_DIR / f"validation_anchor_symprec0p1_shards_{dataset_short(dataset)}.json"


def shard_complete_path(dataset: str) -> Path:
    return STATE_DIR / f"validation_anchor_symprec0p1_shards_{dataset_short(dataset)}.complete.json"
...
def configured_gen_workers() -> int:
    text = os.environ.get("OPENTRY10_GEN_WORKERS", "").strip()
    if text:
        return max(1, int(text))
    return 8


def configured_post_workers() -> int:
    text = os.environ.get("OPENTRY10_POST_WORKERS", "").strip()
    if text:
        return max(1, int(text))
    return 32
...
def action_plan_anchor_shards(dataset: str) -> Callable[[], None]:
    def _run() -> None:
        ...
        for shard_index, start in enumerate(range(0, total, shard_size)):
            ...
            run_name = f"{dataset_short(dataset)}_val_gt_sg_symprec0p1_k100_shard{shard_index:04d}_{start:05d}_{limit:04d}"
        ...
        payload = {
            ...
            "note": (
                "Prompts are built from symprec=0.1 conventionalized validation GT CIFs, matching the recovered "
                "GT-SG test-anchor prompt semantics rather than direct P1 CSV extraction. "
...
@contextmanager
def shard_file_lock(dataset: str, shard: dict[str, Any]):
    run_dir = Path(shard["run_dir"])
    ensure_dir(run_dir)
    lock_path = under_root(run_dir / ".shard.lock")
    with lock_path.open("a+", encoding="utf-8") as lock:
        try:
            fcntl.flock(lock.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise PartialProgress(
                f"{dataset} shard {int(shard['shard_index'])} is already locked by another controller"
            ) from exc
        lock.seek(0)
        lock.truncate()
        lock.write(
...
def run_anchor_shard(dataset: str, plan: dict[str, Any], shard: dict[str, Any]) -> None:
    ...
    for cfg in ANCHOR_K100_CONFIGS:
        ...
        run_logged(stage_id, cmd, max_attempts=3, oom_worker_arg="--workers")
...
def action_generate_anchor_shards(dataset: str) -> Callable[[], None]:
    def _run() -> None:
        ...
        for shard in pending:
            try:
                with shard_file_lock(dataset, shard):
...
def stages() -> list[Stage]:
    return [
        ...
        Stage(
            "prepare_validation_gt_cifs_symprec0p1",
        ...
        Stage(
            "prepare_validation_anchor_symprec0p1_mp20",
        ...
        Stage(
            "prepare_validation_anchor_symprec0p1_mpts52",
        ...
        Stage(
            "plan_validation_anchor_symprec0p1_mp20_shards",
            action_plan_anchor_shards("mp_20"),
            [val_run_dir("mp_20") / "run_meta.json", val_run_dir("mp_20") / "tars/true.tar.gz"],
            [shard_plan_path("mp_20")],
        ),
        Stage(
            "generate_validation_anchor_symprec0p1_mp20_shards",
        ...
        Stage(
            "assemble_validation_anchor_symprec0p1_mp20",
...
def init_layout() -> dict[str, Any]:
    ...
    if state and (state.get("write_root") != str(ROOT) or "opentry9" in str(state.get("run_id", ""))):
        write_json(
            STATE_DIR / "copied_opentry9_state_quarantine.json",
            {
                "created_at": now_iso(),
                "reason": "opentry_9 state was copied into opentry_10; controller is reinitialized to avoid stale resume skips and opentry_9 paths.",
                "previous_run_id": state.get("run_id"),
                "previous_write_root": state.get("write_root"),
                "previous_stage_count": len(state.get("stages", {})) if isinstance(state.get("stages"), dict) else None,
            },
        )
        state = {}
    if not state:
        state = {
            "run_id": "opentry10_" + datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S"),
            "created_at": now_iso(),
            "write_root": str(ROOT),
            "note": "opentry_10 prompt execution. Writes are restricted by this controller to the opentry_10 root.",
            "stages": {},
        }
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Resumable opentry_10 controller.",
