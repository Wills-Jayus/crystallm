...
def _structure_formula_key(structure: Structure) -> str:
...
def _multiplicity_counter(record: SymCifRecord) -> Counter[tuple[str, int, str]]:
    return Counter((s.element, int(s.multiplicity), s.letter) for s in record.sites)


def validate_roundtrip_v2(
    ...
    try:
        ...
        result["formula_consistent"] = _structure_formula_key(rt) == _composition_key(original.cell_formula)
        ...
        counts = Counter()
        for rep in seen:
            indices = [i for i, value in enumerate(equiv) if int(value) == rep]
