...
ARTIFACT_DIR = OUT_DIR / "artifacts" / "exp2r_diverse_pairchem_assignment_posterior"
...
DEFAULT_ASSIGNMENT_MODES = "balanced,prior_strong,source_light,sg_orbit,chem_diverse"
...
def generate_payloads(args: argparse.Namespace, pair_priors: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any], dict[str, Any]]:
    ...
    if int(args.generation_workers) <= 1:
        ...
        for si, (_sid, res) in enumerate(iterator, start=1):
            ...
            if si % 200 == 0:
                print(f"[exp2r-diverse-pairchem] rendered {si}/{len(selected_sids)}", flush=True)
    else:
...
def report_body(result: dict[str, Any]) -> str:
    ...
    gate = result["gate"]
    return f"""## opentry_14 实验 2r：diverse pair-chem assignment posterior
...
- 方法变化：保留 Exp2o pairwise local-chemistry pool，并把 diverse assignment 作为 supplement 与 Exp2o candidates 合并后统一 structural rank；diverse generator 从单一 balanced score 改成 `{','.join(result['method']['assignment_modes'])}` 多模式，每个模式先 beam search，再跨模式去重并按 Hamming distance 保留 diversity。推理仍只用 train split 元素/轨道频率、元素表、局部 pair-distance prior 和 generated structure 字段；不使用 match/RMSD/StructureMatcher label、GT-WA、GT-skeleton、test true CIF 或 official feedback，也不是 RF/HGB/阈值 scorer。
...
- oracle 诊断：diverse pair-chem-only match/skelmatch samples `{result['oracle_diagnostic']['pairchem_match_samples']}` / `{result['oracle_diagnostic']['pairchem_skelmatch_samples']}`；相对 Exp2o 新增 match/skelmatch `{result['oracle_diagnostic']['new_match_samples_vs_exp2o']}` / `{result['oracle_diagnostic']['new_skelmatch_samples_vs_exp2o']}`；union upper bound match@50 `{pct(result['oracle_diagnostic']['union_match50_upper_bound'])}`、conversion `{pct(result['oracle_diagnostic']['union_conversion50_upper_bound'])}`。
...
def main() -> int:
    ...
    result_filename = f"experiment_2r_diverse_pairchem_assignment_posterior{suffix}.json"
    write_jsonl(ARTIFACT_DIR / f"generated_diverse_pairchem_assignment_meta{suffix}.jsonl", generation_meta)
    ...
    with ProcessPoolExecutor(max_workers=max(1, int(args.eval_workers))) as pool:
        ...
        for i, fut in enumerate(as_completed(futures), start=1):
            ...
            if i % 200 == 0:
                print(f"[exp2r-diverse-pairchem] evaluated {i}/{len(futures)}", flush=True)
    ranked_pairchem = assign_structural_ranks(evaluated, int(args.top_k))
    write_jsonl(ARTIFACT_DIR / f"evaluated_diverse_pairchem_assignment_candidates{suffix}.jsonl", ranked_pairchem)
    ...
    write_jsonl(ARTIFACT_DIR / f"combined_exp2o_plus_diverse_pairchem_candidates{suffix}.jsonl", combined_pairchem)
    ...
    diverse_sets = sample_sets([r for r in ranked_pairchem if int(r.get("row_count") or 0) >= 7])
    ...
    oracle = {
        ...
        "diverse_only_match_samples": len(diverse_sets["match"]),
        "diverse_only_skelmatch_samples": len(diverse_sets["skelmatch"]),
    ...
    result = {
        "experiment": "opentry_14_exp2r_diverse_pairchem_assignment_posterior",
        "result_filename": result_filename,
        "time": now_iso(),
        "dataset": "mpts_52",
        "split": "val",
        "method": {
            "name": "exp2o_pairchem_plus_diverse_assignment_supplement",
            "top_skeletons": int(args.top_skeletons),
            "assignment_prelimit": int(args.assignment_prelimit),
            "assignment_limit": int(args.assignment_limit),
            "assignment_beam_width": int(args.assignment_beam_width),
        ...
        "data_scale": {
            ...
            "evaluated_diverse_pairchem_assignment_candidates": len(ranked_pairchem),
        ...
        "diverse_assignment_only": {"rows_ge7": summarize([r for r in ranked_pairchem if int(r.get("row_count") or 0) >= 7]) if ranked_pairchem else {}},
        ...
        "artifacts": {
            "generated_meta": str(ARTIFACT_DIR / f"generated_diverse_pairchem_assignment_meta{suffix}.jsonl"),
            "evaluated_candidates": str(ARTIFACT_DIR / f"evaluated_diverse_pairchem_assignment_candidates{suffix}.jsonl"),
            "combined_candidates": str(ARTIFACT_DIR / f"combined_exp2o_plus_diverse_pairchem_candidates{suffix}.jsonl"),
    ...
    if not args.skip_report:
        marker = "<!-- OPENTRY14_EXP2R_DIVERSE_PAIRCHEM_ASSIGNMENT_POSTERIOR -->"
