...
ARTIFACT_DIR = OUT_DIR / "artifacts" / "exp3k_broad_chgnet_chemical_relax"
...
from run_exp3j_chgnet_after_exp2j import (  # noqa: E402
    ...
    pct,
    pp,
    regenerate_selected_chemical_tasks,
    write_json,
    write_jsonl,
)
    ...
    for variant in ("append_valid_chgnet_after_selected", "append_any_chgnet_after_selected", "replace_invalid_or_collision_if_chgnet_valid"):
        ...
        for sid in sorted(by_sid):
            ...
            for row in by_sid[sid]:
                ...
                is_chemical = str(row.get("geometry_source")) == "chemical_site_order_source_geometry"
    ...
    if payloads:
        with ProcessPoolExecutor(max_workers=max(1, int(eval_workers))) as pool:
            ...
            for i, fut in enumerate(as_completed(futures), start=1):
                ...
                if i % 200 == 0:
                    print(f"[exp3k-eval] evaluated samples {i}/{len(futures)}", flush=True)
    return evaluated
...
def report_body(result: dict[str, Any]) -> str:
    ...
    return f"""## opentry_14 实验 3k：Broad CHGNet relax on Exp2j chemical candidates
...
- baseline：Exp2j best `{result['baseline_exp2j']['best_variant']}` rows>=7 match@50 `{pct(baseline.get('match@50'))}`、conversion `{pct(baseline.get('skeleton_to_match_conversion@50'))}`、collision `{pct(baseline.get('collision_rate'))}`。Exp3 pass line requires conversion `{pct(gate['minimum_standard']['rows_ge7_conversion50'])}`。
...
def main() -> int:
    ...
    if tasks:
        with ProcessPoolExecutor(max_workers=max(1, int(args.chgnet_workers)), initializer=init_chgnet_worker, initargs=(devices,)) as pool:
            ...
            for i, fut in enumerate(as_completed(futures), start=1):
                ...
                if i % 500 == 0:
                    print(f"[exp3k-chgnet] optimized {i}/{len(futures)}", flush=True)

    accepted_with_cif = [dict(r["candidate"], pool_source="exp3k_broad_chgnet") for r in opt_results if bool(r.get("accepted")) and r.get("candidate")]
    write_jsonl_gz(ARTIFACT_DIR / "chgnet_optimized_broad_candidates_with_cif.jsonl.gz", accepted_with_cif)
    ...
    write_jsonl(ARTIFACT_DIR / "evaluated_chgnet_optimized_broad_candidates.jsonl", evaluated_optimized)
    ...
    if passed:
        ...
        reason = "Broad CHGNet relaxation adds at least +2pp conversion over Exp2j while preserving SG/exact-cover and improving collision/local proxy."
        next_step = "Proceed to Exp4 multi-hypothesis free-parameter generator + inference-safe critic using Exp3k as the local optimizer stage."
    else:
        verdict = "fail_local_optimizer_not_conversion_limited"
        reason = "Broad CHGNet relaxation does not deliver the required +2pp conversion lift, so local energy/packing is not the main remaining bottleneck."
    ...
    result = {
        "experiment": "opentry_14_exp3k_broad_chgnet_chemical_relax",
        "time": now_iso(),
        "dataset": "mpts_52",
        "split": "val",
        "method": {
            "name": "broad_chgnet_position_relax_on_exp2j_chemical_candidates",
            "base_result": str(EXP2J_RESULT),
            "base_variant": best_variant_2j,
            "selection_rule": "rows>=7 chemical candidates with formula_ok, space_group_ok, exact_cover_retained, rank<=max_rank and per-sample top-N",
        ...
        "artifacts": {
            ...
            "optimized_candidates_with_cif": str(ARTIFACT_DIR / "chgnet_optimized_broad_candidates_with_cif.jsonl.gz"),
            "evaluated_optimized_candidates": str(ARTIFACT_DIR / "evaluated_chgnet_optimized_broad_candidates.jsonl"),
            "selected_variants": selected_paths,
        },
        "runtime_seconds": time.time() - started,
    }
    write_json(RESULT_DIR / "experiment_3k_broad_chgnet_chemical_relax.json", result)
    body = report_body(result)
    marker = "<!-- OPENTRY14_EXP3K_BROAD_CHGNET_CHEMICAL_RELAX -->"
    append_or_replace(REPORT_PATH, marker, body)
    append_or_replace(LOCAL_REPORT_PATH, marker, body)
    print(json.dumps({"result": str(RESULT_DIR / "experiment_3k_broad_chgnet_chemical_relax.json"), "best_variant": best_variant, "gate": result["gate"], "decision": result["decision"], "diagnostics": diag}, ensure_ascii=False, indent=2))
    return 0
